    ...
    public static class CollectionExtensions
    {
        ...
        public static void Patch<T>(this ICollection<T> source, ICollection<T> target, IEqualityComparer<T> comparer, Action<T, T> patch)
        {
            Action<EntryState, T, T> patchAction = (state, x, y) =>
            {
                ...
                {
                    var auditableEntity = x as IDeletableEntity;
                    if (auditableEntity == null)
                        target.Remove(x);
                    else
                    {
                        if (!auditableEntity.Deleted)
                        {
                            patch(x, y);
                            auditableEntity.Deleted = true;
                        }
                    }
                }
            };
        ...
        ///// <summary>
        ///// Сравнение элементов коллекции через идентификатор
        ///// </summary>
        //public static void CompareToEntity<T>(this ICollection<T> source, ICollection<T> target, IEqualityComparer<T> comparer, Action<EntryState, T, T> action) where T : Entity
        //{
        //    var entityByIdComparer = new EntityByIdComparer<T>();
        //    //Change
        //    foreach (var sourceItem in source)
        //    {
        //        var targetItem = target.FirstOrDefault(x => x.Id == sourceItem.Id);
        //        if (targetItem != null && !targetItem.Equals(default(T)) && comparer.Equals(x, sourceItem))
        //        {
        //            targetItem = target.FirstOrDefault(x => );

        //            action(EntryState.Modified, sourceItem, targetItem);
        //        }
        //    }
        //    //Add
        //    foreach (var newItem in source.Except(target, entityByIdComparer))
        //    {
        //        action(EntryState.Added, newItem, newItem);
        //    }
        //    //Remove
        //    foreach (var removedItem in target.Except(source, entityByIdComparer).ToArray())
        //    {
        //        action(EntryState.Deleted, removedItem, removedItem);
        //    }
        //}

        //class EntityByIdComparer<T> : IEqualityComparer<T> where T : Entity
        //{
