	        ...
	        controller: ['$scope', 'platformWebApp.authService', function ($scope, authService) {
	            ...
	            $scope.ok = function () {
	                ...
	                authService.login($scope.user.email, $scope.user.password, $scope.user.remember).then(function (loggedIn) {
	                    $scope.loginProgress = false;
	                    if (!loggedIn) {
	                        $scope.authError = 'Ошибка входа пользователя';
	                    }
	                }, function (x) {
	                    $scope.loginProgress = false;
	                    if (angular.isDefined(x.status)) {
	                        if (x.status == 401) {
	                            $scope.authError = 'Имя пользователя или пароль указаны неверно.';
	                        } else {
	                            $scope.authError = 'Ошибка входа пользователя (код: ' + x.status + ').';
	                        }
	                    } else {
	                        $scope.authError = 'Ошибка входа пользователя ' + x;
	                    }
	                });
	            };
