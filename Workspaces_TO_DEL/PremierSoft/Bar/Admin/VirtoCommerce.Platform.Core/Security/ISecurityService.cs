...
namespace VirtoCommerce.Platform.Core.Security
{
    public interface ISecurityService
    {
        ...
        bool IsUserCompanyManager(string userName, out int companyId);
        int? GetCompanyId(string userName);
        bool UserHasAnyPermission(string userName, string[] scopes, params string[] permissionIds);
        bool UserHasAnyPermission(ApplicationUserExtended user, string[] scopes, params string[] permissionIds);
        Permission[] GetAllPermissions();
        Permission[] GetUserPermissions(string userName);

        void RemoveUsersFromCacheByRole(string roleId);     // PS
    }
}
