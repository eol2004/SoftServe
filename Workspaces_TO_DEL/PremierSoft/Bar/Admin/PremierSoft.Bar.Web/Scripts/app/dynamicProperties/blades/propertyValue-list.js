...
.controller('platformWebApp.propertyValueListController', ['$scope', 'platformWebApp.bladeNavigationService', 'platformWebApp.dialogService', 'platformWebApp.settings', 'platformWebApp.dynamicProperties.dictionaryItemsApi', '$translate', function ($scope, bladeNavigationService, dialogService, settings, dictionaryItemsApi, $translate) {
    var blade = $scope.blade;
    blade.updatePermission = 'platform:dynamic_properties:items_update';
    ...
    $scope.getDisplayName = function (property) {
        var existName = _.find(property.displayNames, function (dn) { return dn.locale.substr(3).toLowerCase() === currentLanguage; });
