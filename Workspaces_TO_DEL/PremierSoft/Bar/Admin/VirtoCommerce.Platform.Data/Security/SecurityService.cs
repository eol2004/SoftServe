    ...
    public class SecurityService : ServiceBase, ISecurityService
    {
        ...
        public async Task<SecurityResult> UpdateAsync(ApplicationUserExtended user)
        {
            ...
            using (var userManager = _userManagerFactory())
            {
                ...
                if (dbUser == null)
                {
                    if (string.IsNullOrEmpty(user.UserName))        // PS: Если пользователь не имеет логина для входа на сайт   
                        result = new SecurityResult { Succeeded = true };
                    else
                    {
                        // PS: Для пользователя создаем учетную запись для входа на сайт
                        //Update ASP.NET indentity user
                        dbUser = user.ToIdentityModel();
                        user.Id = dbUser.Id;
                        ...
                        if (string.IsNullOrEmpty(user.Password))
                            ...
                            identityResult = await userManager.CreateAsync(dbUser, user.Password);
                        result = identityResult.ToCoreModel();
                    }
                    //clear cache
                    RemoveUserFromCache(user.Id, string.Empty);
                }
                else
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        IdentityResult identityResult;
                        user.Patch(dbUser);
                        if (string.IsNullOrEmpty(user.UserName))      // PS: Если имя пользователя для входа на сайт не задано
                        {
                            dbUser.UserName = null;
                            identityResult = await userManager.DeleteAsync(dbUser);     // Запрещаем пользователю входить на сайт
                        }
                        else
                        {
            ...
            if (result.Succeeded)
            {
                ...
                using (var repository = _platformRepository())
                {
                    var targetDbAcount = repository.GetAccountById(user.Id, UserDetails.Full);  // PS
        ...
        public async Task DeleteAsync(string[] ids)
        {
            using (var userManager = _userManagerFactory())
            {
                foreach (var id in ids)
                {
                    ...
                    var dbUser = await userManager.FindByIdAsync(id);      // PS Для поддержки пользователей без входа на сайт
                    if (dbUser != null)
                    {
                    ...
                    using (var repository = _platformRepository())
                    {
                        var account = repository.GetAccountById(id, UserDetails.Reduced);
                        if (account != null)
                        {
                    ...
                    //clear cache
                    RemoveUserFromCache(id, userName);
                }
            }
        }
        ...
        public bool IsUserCompanyManager(string userName, out int companyId)
        {
            var user = FindByName(userName, UserDetails.Reduced);

            if (user != null && user.UserState == AccountState.Approved && user.CompanyId.HasValue)
            {
                if (string.Equals(user.UserType, AccountType.CompanyManager.ToString(), StringComparison.InvariantCultureIgnoreCase) ||
        ...
        public int? GetCompanyId(string userName)
        {
            ...
            if (user != null && user.UserState == AccountState.Approved)
        ...
        public bool UserHasAnyPermission(string userName, string[] scopes, params string[] permissionIds)
        {
            ...
            return UserHasAnyPermission(user, scopes, permissionIds);
        }
        ...
        public bool IsAdministrator(string userName)
        {
            ...
            return result && user.IsAdministrator;
        }
        ...
        public Permission[] GetUserPermissions(string userName)
        {
            ...
            if (user == null)
                return Enumerable.Empty<Permission>().ToArray();

            // PS Объединяем совпадающие разрашения из разных ролей
            var result = new List<Permission>();
            foreach (var pair in user.Roles.SelectMany(x => x.Permissions).GroupBy(x => x.Id))
            {
                var permission = pair.FirstOrDefault();
                if (permission == null)
                    continue;
                if (pair.Count() > 1)
                {
                    var assignedScopes = pair.SelectMany(x => x.AssignedScopes).ToList();
                    permission.AssignedScopes = assignedScopes;
                }
        ...
        private async Task<ApplicationUser> GetApplicationUserByIdAsync(string userId)
        {
            ...
            var result = await _cacheManager.GetAsync(cacheRegion, cacheRegion, async () =>
            {
                ...
                if (user == null)        // PS Для поддержки пользователей без входа на сайт
                {
                    using (var repository = _platformRepository())
                    {
                        var dbUser = repository.GetAccountById(userId, UserDetails.Reduced);
                        user = dbUser.ToIdentityModel();
                    }
                }
                return user;
            });
        ...
        private ApplicationUserExtended GetUserExtended(ApplicationUser applicationUser, UserDetails detailsLevel)
        {
            ...
            if (applicationUser != null)
            {
                ...
                result = _cacheManager.Get(cacheRegion + ":" + detailsLevel, cacheRegion, () =>
                {
                    ...
                    using (var repository = _platformRepository())
                    {
                        Model.AccountEntity user;
                        if (string.IsNullOrEmpty(applicationUser.UserName))     // PS: Если пользователь создан только для входа на кассу
                            user = repository.GetAccountById(applicationUser.Id, detailsLevel);
                        else
                            user = repository.GetAccountByName(applicationUser.UserName, detailsLevel);
        ...
        public void RemoveUsersFromCacheByRole(string roleId)    // PS
        {
            using (var repository = _platformRepository())
            {
                var accounts = repository.Accounts.Where(x => x.RoleAssignments.Any(y => y.RoleId == roleId));
                foreach (var account in accounts)
                    RemoveUserFromCache(account.Id, account.UserName);
            }
        }
    }
}
